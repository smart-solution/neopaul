openerp.project_planning_tool = function(instance){
    var QWeb = instance.web.qweb;

    instance.web.client_actions.add('project.planning.ui', 'instance.project_planning_tool.ProjectWidget');

    instance.project_planning_tool.ProjectWidget = instance.web.Widget.extend({
            ...
            if(! date_start){
                ...
                this.project_task_dataset.read_slice(([]), {}).then(function(records) {
                    if(! records.length){
                        ...
                        $(".project_data").html(QWeb.render('Project-User',{'users': self.users}))
                        ...
                        $(".week_project_table").html(QWeb.render('week-data',{'week_datas': week_data, 'users':self.users}))
                        ...
                        $columns.sortable({
                            ...
                            start: function(event, ui) {
                                self.old_user = $(".week_project_table tr td:eq("+$(ui.item).closest("td").index()+')')
                            ...
                            stop: function(event, ui) {
                                ...
                                self.new_user = $(".week_project_table tr td:eq("+$(ui.item).closest("td").index()+')') 
                                ...
                                if(self.new_user && self.new_user != self.old_user){
                                    self.project_task_dataset = new instance.web.DataSetSearch(self, 'project.task', {}, []);
                                    self.project_task_dataset.write(parseInt(self.current_record_id), {'user_id':parseInt(self.new_user.attr("id")),'date_start':self.drag_date});
                                }
                                self.setup_color_picker()
                                setTimeout(function() {
...
//                                  ui.item.find('*').off('click.prevent');
                        ...
                        _.each(self.$el.find('.oe_kanban_card'),function(card){
                            $(card).dblclick(function(){
                                console.log("DDDDDD",$(this).data('project_id'))
        ...
        setup_color_picker: function() {
            ...
            if ($el.length) {
                ...
                $el.on('click', 'a', function(ev) {
                    ...
                    var endOfWeek = instance.web.date_to_str(date_to)
                    if(self.load){
                        setTimeout(function() { self.show_project(self.datestart_widget.get_value(),self.dateto_widget.get_value()) },100)
                    }else{
                        setTimeout(function() { self.show_project(startOfWeek,endOfWeek) },100)
                    }
                });
            }
        },
